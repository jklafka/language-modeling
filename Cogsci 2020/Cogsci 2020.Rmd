...
annotations <- tibble(position = c(4, 4), surprisal = c(4, 2),
                      language = c("bnc", "switchboard"), gram = "unigram") %>%
    mutate(gram = factor(gram, levels = c("unigram", "trigram")))


tidy_b %>% 
    filter(gram %in% c("unigram", "trigram"), language %in% c("bnc", "switchboard")) %>% 
