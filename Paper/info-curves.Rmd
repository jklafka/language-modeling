...
```{r check for langs to do}
crossval_langs <- read_csv(here("Data/crossval_barycenters.csv")) %>% 
  filter(source == "wikipedia") %>% 
  distinct(language)

langs <- read_csv(here("languages.txt")) %>% 
  distinct(language) 

langs %>%
  anti_join(crossval_langs, by = "language") %>%
  pull(language)
```
...
b <- read_csv(here("Data/bootstrapped_crossval_barycenters.csv"))

b <- read_csv(here("Data/crossval_barycenters.csv")) %>%
  group_by(language, source, gram) %>%
  slice(1) %>%
  left_join(families, by = "language")

#write_csv(b, "Data/families_barycenters.csv", col_names = FALSE)
...
tidy_b <- b %>%
     pivot_longer(-c(language, source, gram, strap), 
  ...
  summarise(ci_upper = quantile(surprisal, .95),
            ci_lower = quantile(surprisal, .05),
            surprisal = mean(surprisal))


non_wiki_b <- read_csv(here("Data/crossval_barycenters.csv")) %>%
              # col_names = c("1", "2", "3", "4", "5",
              #               "language", "source", "gram")) %>%
  filter(source != "wikipedia")
...
sbc_b <- read_csv(here("Data/sbc_barycenters.csv"),
              col_names = c("1", "2", "3", "4", "5",
                            "language", "source", "gram", "sample")) %>%
  filter(source != "wikipedia")

tidy_sbc_b <- sbc_b %>%
     pivot_longer(-c(language, source, gram, sample), 
                  names_to = "position", values_to = "surprisal") %>%
     mutate(position = as.numeric(position),
            gram = factor(gram, levels = c("unigram", "trigram")))
...
```{r}
tidy_sbc_b %>% 
  group_by(language, gram, position) %>%
  summarise(ci_upper = quantile(surprisal, .975), 
            ci_lower = quantile(surprisal, .025),
            surprisal = mean(surprisal)) %>%
    ggplot(aes(x = position, y = surprisal, color = gram)) + 
    geom_pointrange(aes(ymin = ci_lower, ymax = ci_upper), size = .75) + 
    geom_line()
...
#   ungroup() %>% 
#   write_csv(here("Data/5barycenters_fam.csv"))
```
...
no_trigrams <- tidy_b %>%
  ungroup() %>%
  select(language, gram) %>%
  distinct() %>%
  anti_join(langs, ., by = "language")

unigrams <- tidy_b %>%
  filter(position == 1, gram == "unigram") %>%
  pull(language)

no_unigrams <- langs %>%
  filter(!language %in% unigrams) %>%
  pull(language)
...
ggplot(genealogy_barycenters %>%
         filter(family %in% c("Indo-European", "Austronesian", 
                             "Niger-Congo", "Dravidian"),
                genus == "mean"), 
...
ggplot(genealogy_barycenters %>%
         filter(genus %in% c("Germanic", "Slavic", 
                             "Romance", "Greek",
                             "Baltic")), 
       aes(x = position, y = surprisal, color = genus,
                               group = genus)) + 
  geom_point() + 
  geom_line() + 
  facet_wrap(~gram) + 
...
Non-uniformity across languages
```{r non-uniform}
lang_surprisals <- families_b %>%
  filter(source == "wikipedia", position) %>%
...
gram_similarities <- lang_surprisals %>%
  group_by(gram, position) %>%
  summarise(surprisal = sd(surprisal)) %>%
...
```{r}
family_means <- genealogy_barycenters %>%
  filter(genus == "mean", family != "mean") %>%
  rename(family_surprisal = surprisal) %>%
  select(-genus)

genus_means <- genealogy_barycenters %>%
  filter(genus != "mean", family != "mean") %>%
  rename(genus_surprisal = surprisal) 

overall_means <- genealogy_barycenters %>%
  filter(genus == "mean", family == "mean") %>%
  rename(overall_surprisal = surprisal) %>%
  select(-genus, -family)

families_b_scaled <- families_b %>%
  filter(!is.na(family), !is.na(genus)) %>%
  group_by(gram, family, genus, language) %>%
  mutate(surprisal = scale(surprisal, scale = FALSE)) %>%
  ungroup() 

family_n <- families_b_scaled %>%
  distinct(gram, language, family) %>%
  group_by(gram, family) %>%
  count()

genus_n <- families_b_scaled %>%
  distinct(gram, language, family, genus) %>%
  group_by(gram, family, genus) %>%
  count()

SSG_family <- family_means %>%
  left_join(family_n, by = c("family", "gram"))%>%
  left_join(overall_mean, by = c("gram", "position")) %>%
  group_by(gram, family, n) %>%
  nest() %>%
  mutate(dist = map(data, ~dtwDist(t(.x$family_surprisal), 
                                   t(.x$overall_surprisal)))) %>%
  select(-data) %>%
  unnest(cols = c(dist)) %>%
  mutate(dist = dist * n) %>%
  ungroup() %>%
  group_by(gram) %>%
  summarise(group = sum(dist))

SSG_genus <- genus_means %>%
  left_join(genus_n, by = c("family","genus", "gram"))%>%
  left_join(overall_mean, by = c("gram", "position")) %>%
  group_by(gram, family, genus, n) %>%
  nest() %>%
  mutate(dist = map(data, ~dtwDist(t(.x$genus_surprisal), 
                                   t(.x$overall_surprisal)))) %>%
  select(-data) %>%
  unnest(cols = c(dist)) %>%
  mutate(dist = dist * n) %>%
...
SST <- families_b_scaled %>%
  left_join(overall_means, by = c("gram", "position")) %>%
  group_by(gram, family, genus, language) %>%
  nest() %>%
  mutate(dist = map(data, ~dtwDist(t(.x$surprisal), 
                                   t(.x$overall_surprisal)))) %>%
  select(-data) %>%
  unnest(cols = c(dist)) %>%
  group_by(gram) %>%
...
Fs_family <- left_join(SSG_family, SST, by = c("gram")) %>%
  mutate(e = total - group) %>%
  mutate(df_g = unique(families_b_scaled$family) %>% length - 1,
         df_e = unique(families_b_scaled$language) %>% length() - df_g - 1,
         msg = group / df_g,
         mse = e / df_e,
         f = msg / mse,
         p = pf(f, df_g, df_e, lower.tail = FALSE))

Fs_genus <- left_join(SSG_genus, SST, by = c("gram")) %>%
  mutate(e = total - group) %>%
  mutate(df_g = unique(families_b_scaled$genus) %>% length - 1,
         df_e = unique(families_b_scaled$language) %>% length() - df_g - 1,
         msg = group / df_g,
         mse = e / df_e,
         f = msg / mse,
...
families_b %>%
    distinct(language, gram, position, .keep_all = T) %>%
    filter(family == "Niger-Congo") %>%
    ggplot(aes(x = position, y = surprisal, color = language, label = language,
               group = interaction(language, family))) +
    facet_grid(gram ~ genus, scales = "free_y") +
    geom_point() + 
    geom_line() +
    # scale_color_ptol() + 
