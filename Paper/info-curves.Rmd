...
families %>%
  group_by(family, genus, language) %>%
  distinct() %>%
...
```{r}
family_barycenters <- read_csv(here("Data/by_family_barycenters.csv")) %>%
  filter(!is.na(family)) %>%
  pivot_longer(-c(family, gram), 
                  names_to = "position", values_to = "surprisal") %>%
  mutate(position = as.numeric(position))

genus_barycenters <- read_csv(here("Data/by_genus_barycenters.csv")) %>%
  filter(!is.na(genus)) %>%
  pivot_longer(-c(genus, gram), 
                  names_to = "position", values_to = "surprisal") %>%
  mutate(position = as.numeric(position))

ggplot(family_barycenters %>%
         filter(family %in% c("Indo-European", "Austronesian", 
                             "Niger-Congo", "Dravidian")), 
       aes(x = position, y = surprisal, color = family,
                               group = family)) + 
  geom_point() + 
  geom_line() + 
  facet_wrap(~gram) + 
...
families_b_scaled <- families_b %>%
  ...
  mutate(surprisal = scale(surprisal, scale = FALSE)) %>%
  ungroup() %>%
  left_join(family_barycenters %>% rename(family_surprisal = surprisal),
            by = c("family", "gram", "position")) %>%
  group_by(gram, family, language) %>%
  nest() %>%
  mutate(dist = map(data, ~dtwDist(surprisal, family_surprisal)))

family_means <- families_b %>%
  group_by(gram, position, family) %>%
  summarise(family_n = n())

overall_mean <- families_b %>%
  group_by(gram, position) %>%
  summarise(overall_surprisal = mean(surprisal))

SSG <- family_means %>%
  left_join(overall_mean, by = c("gram", "position")) %>%
  mutate(dist = family_n * (family_surprisal - overall_surprisal)^2) %>%
  summarise(dist = sum(dist)) %>%
  summarise(group = sum(dist))

SST <- families_b %>%
  left_join(overall_mean, by = c("gram", "position")) %>%
  mutate(dist = (surprisal - overall_surprisal)^2) %>%
  group_by(gram, position) %>%
  summarise(dist = sum(dist)) %>%
  summarise(total = sum(dist))

SSE <- left_join(SSG, SST, by = c("gram")) %>%
  mutate(e = total - group) %>%
  mutate(df_g = unique(family_means$genus) %>% length - 1,
         df_e = unique(families_b$language) %>% length() - df_g - 1,
         msg = group/df_g,
         mse = e/df_e,
         f = msg/mse)
