...
langs <- read_csv(here("Data/final_features.csv"), show_col_types = FALSE) %>%
  pull(language)
...
raw_wals_features <- values %>%
  inner_join(iso_to_language, by = "iso") %>%
  #semi_join(langs, by = "language", copy = TRUE) %>%
  select(-language_id, -iso) %>%
  distinct(language, .keep_all = TRUE) %>%
  select(language, x1a:x120a)

write_csv(raw_wals_features, here("Data/mdpi_paper/raw_wals_features.csv"))

# get all currently valued features
wals_features <- read_csv(here("Data/mdpi_paper/wals_features.csv"),
                          show_col_types = FALSE) %>%
  clean_names() %>%
...
# imputation
imputed_wals <- raw_wals_features %>%
  column_to_rownames("language") %>%
  select(x1a:x120a) %>%
  mutate_all(as_factor) %>%
  MIMCA(ncp = 20, threshold = 1e-1, maxiter = 1000)
...
wals_features <- read_csv(here("Data/mdpi_paper/raw_wals_features.csv"),
                          show_col_types = FALSE) %>%
...
wals_distance <- function(language1, language2) {
  wals_features %>%
    ...
    summarise(wals_dist = mean(wals_dist)) %>%
...
wals_space %>%
  write_csv(here("Data/mdpi_paper/raw_wals_distances_mean.csv"))

wals_type_space <- map_dfr(1:nrow(langs_pairwise), ~wals_type_distance(langs_pairwise[.x,"language1"],
                                          langs_pairwise[.x,"language2"]))
wals_type_space %>%
  write_csv(here("Data/mdpi_paper/raw_wals_type_distances_mean.csv"))
